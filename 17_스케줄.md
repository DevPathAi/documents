...
- [x] web/admin은 목 API 중심(2026-06-16 기준) · **모바일(P6) 라우트 8종 구현**(로그인·온보딩·홈·학습·커뮤니티 3종·알림, 목 데이터 기반, 2026-06-27) · **랜딩(P7) 검증용 정적 페이지 가동**(Cloudflare Functions + 실 Claude 호출, 2026-06-24, 장기 Jaspr 랜딩은 `feat/p7-landing-jaspr` 별도 진행) — 상세 [42_전체_정합성_점검_2차](./42_전체_정합성_점검_2차.md) §4.3
...
- [x] 학습경로·콘텐츠·Sandbox·AI 리뷰·멘토·커뮤니티 도메인 API **구현 완료**(백엔드 총 **80 엔드포인트**, 2026-07-27 실측 = 3차 70 + 베타 광고 10) — 상세 [46_전체_정합성_점검_3차](./46_전체_정합성_점검_3차.md) §5. (증가분 +18: 베타 게이팅·설정/동의·마이페이지 avatar/profile·재참여·게스트 진단 / +10: 베타 광고 [04 §11](./04_API_명세서.md))

**문서**
- [x] 01~38 작성

### 0.1 현재 구현 현황 스냅샷 (2026-07-02, [42_전체_정합성_점검_2차](./42_전체_정합성_점검_2차.md) 기준)

| 영역 | 현재 상태 | 다음 정합성 과제 |
|---|---|---|
| 인증/사용자 | 구현 (platform-svc 26 엔드포인트: auth·admin·beta·consent·avatar/profile·ads) | OAuth provider별 E2E, gateway edge 흐름 확인 |
| 진단 | 구현 (learning-svc 10종, 회원/비회원/claim) | 프론트 실API 전환(기본값 USE_MOCK=true 해제) |
| AI Gateway | dev Ollama gateway 구현(embed/path 2종) | 운영 provider(Claude 등) 전환 |
| 학습경로·콘텐츠 | 구현(learning-svc 학습경로 5종+콘텐츠 3종+internal 2종) | 프론트 실API 전환 |
| Sandbox·AI 리뷰·멘토 | 구현(sandbox-svc 3종, ai-svc review 3종+mentor 1종) | 프론트 실API 전환 |
| 커뮤니티 | 구현(community-svc 11종: Q&A·투표·배지·평판·활동집계) | moderation·에스컬레이션·현상금·검색·북마크 등 후속(TARGET, 미구현) |
| LCS | 구현(lcs-svc 6종: draft/commit/조회/preferences) | sanitize_rules·이벤트 스트림 후속(TARGET) |
| 알림 | 구현(notification-svc device 2종) | 인박스 조회 API, 참여촉진 배치(Build 2+) |
| 모바일(P6)·랜딩(P7) | 모바일 라우트 8종 구현(목 데이터), 랜딩 검증 페이지 가동 | 모바일 실API 전환, 장기 Jaspr 랜딩(진행 중) |
| 평판·배지·남용방지 | 핵심 완료(Build 1~3) | 레벨 게이트 강제, Silver/Gold, IP 기반 sockpuppet |
...
- [ ] **결제**: PortOne 통합 PG 연동(월 14,900원 구독 진입, 다중 수단·빌링키)
...
**평판 기초**
- [x] 평판 엔진(upvote/downvote/채택) + 태그별 평판(`user_tag_reputation`) + 레벨별 권한(15/125/500/1000) + Bronze 배지 9종 + 일일 +40 상한·sockpuppet 탐지 — **핵심 완료**(2026-06-30~07-01, Build 1·2·3): 평판 엔진·태그평판·일일 +40 상한(community-svc PR #13 / shared #30) · Bronze 배지 9종 시드+6종 트리거+조회 API(PR #15 / shared #31) · 자기 글 투표 금지 + 행동 기반 담합 탐지(reputation_events 반복 upvote, 기록만)(PR #17 / shared #32). **후속**: 레벨 게이트 500/1000 미강제, Bronze 3종(FIRST_STEP·EDITOR·COMMUNITY) 시드만(프로필·편집·스트릭 기능 부재로 결선 대기), Silver/Gold 배지, IP·디바이스 기반 sockpuppet·신계정 7일 제한(platform-svc 신호 필요), moderation 자동 제재(suspicion 이벤트 소비자 미구현)
