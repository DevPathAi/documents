...
> DevPath AI 기획·설계·운영 문서 (계획서 / ERD / 아키텍처 / API 명세 / 스케줄 등)
...
산출물이 코드가 아니라 문서다. 절대 조건 2는 **문서 정합성 검증**으로 적용한다.

- 코드·스키마 변경이 근거인 문서 수정은 해당 레포의 **실제 코드를 확인하고** 반영한다(추측 금지).
- 한 사실이 여러 문서에 걸치면(ERD ↔ API 명세 ↔ 화면 정의 등) **교차로 함께 갱신**해 모순을 남기지 않는다.
- 링크·표·mermaid 다이어그램이 깨지지 않는지 확인한다.
- [workflow-guide](https://github.com/DevPathAi/workflow-guide)가 일부 문서를 사이트로 빌드한다 — 파일명/제목 변경 시 `guides.config.json` 영향을 확인한다.
