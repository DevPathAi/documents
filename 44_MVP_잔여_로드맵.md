...
| ① | 결제(PortOne 통합 PG) | §1-2 · §3-1 화면 9 | ❌ 전무 | 대 |
...
- **① 결제 마지막**: 최대·최고 복잡. 마이페이지 뼈대 위에 구독 섹션을 얹고, 유료/무료 게이팅을 전 기능에 반영. **외부 의존(PortOne 가맹 계약·API 키, 사업자 등록)에 게이팅**되므로 나머지가 안정된 뒤 착수하는 것이 리스크상 유리.
...
### ① 결제 (PortOne 통합 PG)
- **backend**(platform, 신규 도메인): 구독(subscription)·결제(payment) 도메인, 유저 `plan/tier` 필드, DB 스키마(subscriptions·payments·plan 이력), PortOne 통합 PG 연동(다중 수단 토스페이·카카오페이·네이버페이·카드 + 빌링키 customer_uid 정기결제·해지·웹훅), 무료→유료 상태 전이. (mock PG 어댑터 선구현→실연동 교체, platform #28 마스터 spec)
- **게이팅**: AI 멘토 횟수 등 유료 기능 제한([27](./27_MVP_설계서.md) 시나리오 4). 기존 quota/killSwitch 표면(프론트 `isQuota`)과 연결.
- **frontend**(`features/billing` 신규 + 마이페이지 구독 섹션): 결제 화면(PortOne 결제창), 무료→유료 전환 모달, 구독 관리(해지)·결제 내역.
- **의존**: ②(마이페이지 구독 섹션 안착), ④(동의/약관 — 결제 약관). 외부: **PortOne 가맹 계약·API 키, 사업자 등록**(런북·게이팅). 산출물: 구독/결제 백엔드 + 결제 화면 + 게이팅. **H1(지불의사) 검증의 핵심.**
...
3. ①(결제)는 외부 의존(PortOne 키·계약) 확보 시점에 착수. 미확보면 백엔드 도메인·스키마·프론트 UI까지 mock/테스트로 선구현하고 실연동만 게이팅.
