...
┌────────────────────────────────────────────────────────────────┐
│                       Client Layer                             │
│   Flutter Web     │  Flutter 3.x (Mobile)                       │
└───────────────┬──────────────────┬─────────────────────────────┘
                └──────┬───────────┘
                       │
                ┌──────▼──────┐
...
graph TB
    ...
    subgraph External
        LLM[LLM Provider<br/>dev: Ollama / prod: Claude]
        GH[GitHub API]
    end
    subgraph Data
        PG[(PostgreSQL 17)]
...
2. **AI Gateway 단일 진입점**: LLM provider 세부사항을 숨긴다. 현재 개발 빌드는 Ollama gateway(`/ai/embed`, `/ai/path/generate`)이고, 운영 목표는 Claude API 호출, 비용 추적, Semantic Cache, Kill-switch를 한 곳에서 처리하는 구조다.
...
├── devpath-platform-svc/   # user/auth · github 수집
├── devpath-notification-svc/ # FCM 디바이스 토큰 · 인앱 알림 인박스 (2026-07-01 platform-svc에서 이관)
├── devpath-learning-svc/   # onboarding · learning(경로 엔진) · content
├── devpath-community-svc/  # post · reputation · badge · abuse
├── devpath-lcs-svc/        # LCS(학습 맥락 자동 첨부) — 스냅샷 draft/commit · preferences
├── devpath-ai-svc/         # ai-gateway(provider abstraction, dev Ollama) · mentor · review
├── devpath-sandbox-svc/    # Docker + gVisor 격리 실행
├── devpath-frontend/       # web·admin·mobile 모두 Flutter (Dart, melos 모노레포)
├── devpath-landing-page/   # 검증용 정적 랜딩(Cloudflare Pages Functions, 장기 Jaspr 랜딩과 별개)
├── devpath-gitops/         # Kubernetes 매니페스트 + ArgoCD + Flyway 마이그레이션 Job
└── devpath-svc-template/   # 신규 devpath-*-svc 스캐폴딩 표준 템플릿
```
...
│  AI Gateway                                                 │
│  ├── 4분류 모델 라우팅                                        │
│  │    (개발: Ollama chat/embed, 운영: Sonnet/Haiku/OpenAI    │
│  │     등 provider 교체 가능 계약)                          │
...
| 007 | AI Gateway provider abstraction | 개발 빌드는 Ollama로 비용 없이 계약 검증, 운영 빌드는 Claude API(Sonnet 주력) 등으로 교체 |
