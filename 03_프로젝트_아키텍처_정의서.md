...
> **버전**: v1.0
> **스타일**: 모놀리식 코어 + 격리된 Sandbox 서비스 + AI Gateway
...
graph TB
    ...
    Sbx --> Docker
    Sbx --> AIGW
    Core --> MySQL & Redis & Kafka
    Auth --> GH
```

### 1.2 아키텍처 핵심 결정

1. **모놀리식 코어 + 격리 서비스**: 팀 규모(12명)에서 MSA는 과도. 단, Sandbox는 보안상 무조건 격리된 별도 서비스.
...
```text
devpath/
├── devpath-api/                # 메인 애플리케이션
│   ├── user/                   # 사용자/OAuth2/JWT
│   ├── github/                 # GitHub 프로필 수집 워커
│   ├── onboarding/             # 적응형 진단 테스트
│   ├── learning/               # 학습 경로 엔진 + 진척도
│   ├── content/                # 콘텐츠 CRUD + 임베딩
│   ├── mentor/                 # AI 멘토 채팅
│   ├── community/              # 5개 게시판 + 평판 + 배지 + 태그 + 학습 맥락 스냅샷
│   │   ├── post/               # Q&A / 자유 / 프로젝트 / 스터디 / 수료자 라운지
...
│   └── common/                 # JWT, AOP, 예외, Outbox
│
├── devpath-ai-gateway/         # Claude 오케스트레이터 + 캐시 + FinOps
├── devpath-sandbox-runner/     # 격리 실행 (Docker + gVisor)
├── devpath-ai-review-worker/   # Kafka Consumer, 코드 리뷰 비동기
│
├── devpath-web/                # React SPA
├── devpath-mobile/             # Flutter (모바일 대시·퀵 캡처)
├── devpath-admin/              # React 관리자 (콘텐츠·FinOps)
...
| 저장소 | 역할 | 근거 |
|--------|------|------|
| **MySQL 8.x** | Source of Truth | 트랜잭션 + 운영 성숙도 |
...
| 002 | MySQL (SSOT) + pgvector 분리 | 벡터는 Postgres가 생태계 우위 |
