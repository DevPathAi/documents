...
MAIN_SHA = "eae5c42ef33b1a633a632766410e63d655b409fb"
TARGET_SHA = "5427fe1e6e5729309687c0ddec3fc1a8b16683f1"
TARGET_TREE = "0c22b0f3db052ab5a99fa7bfa11a7c01fb37091f"
HELPER_BRANCH = "chore/gateway-edge-cors-publish-20260924"
TARGET_BRANCH = "fix/gateway-edge-cors-main-20260924"
ENVIRONMENT = "mission-spine-production-off"
CONTRACT_TEST = "test_gateway_edge_cors_main_publish.py"
TARGET_ROWS = (
    ("M", "apps/devpath-gateway/base/deployment.yaml"),
    ("A", "tests/release/test_gateway_edge_cors.py"),
)
SUBJECT = "release: restore the gateway edge CORS dedupe in the production default-filters"
...
STEP_TARGET_CHECKOUT = "Checkout the exact tested gateway-edge-cors target"
STEP_TARGET_TEST = "Test the exact gateway-edge-cors target"
STEP_MINT = "Mint the production-scoped release App token"
STEP_PUSH = "Fast-forward protected main to the exact tested gateway-edge-cors target"
...
STEP_NAMES = [
    ...
    'Checkout the exact tested gateway-edge-cors target',
    None,
    'Test the exact gateway-edge-cors target',
    ...
    'Fast-forward protected main to the exact tested gateway-edge-cors target',
...
GATE_BODIES = {
    'Prove the exact one-shot helper context': [
        ...
        'test "${helper_paths[1]}" = "tests/release/test_gateway_edge_cors_main_publish.py"',
        'current_main="$(',
    ...
    'Run the publisher contract test on the exact helper bytes': [
        ...
        "python -m unittest discover -s tests/release -p 'test_gateway_edge_cors_main_publish.py'",
    ],
    'Authenticate the live protected environment and this approval': [
        'set -euo pipefail',
        'environment_json="$(',
    ...
    'Test the exact gateway-edge-cors target': [
        ...
        'test "$(git show -s --format=%s HEAD)" = \'release: restore the gateway edge CORS dedupe in the production default-filters\'',
        ...
        'expected_listing="$(printf \'%s\\n\' $\'M\\tapps/devpath-gateway/base/deployment.yaml\' $\'A\\ttests/release/test_gateway_edge_cors.py\')"',
...
- name: Fast-forward protected main to the exact tested gateway-edge-cors target
  env:
    GH_TOKEN: ${{ steps.app_token.outputs.token }}
    APP_ID: ${{ secrets.GITOPS_RELEASE_APP_ID }}
    APP_SLUG: ${{ steps.app_token.outputs.app-slug }}
...
class GatewayEdgeCorsMainPublishTest(unittest.TestCase):
    @classmethod
    ...
    def test_job_is_the_only_job_and_is_fenced(self) -> None:
        ...
        self.assertEqual(
            {"group": "gateway-edge-cors-main-publish", "cancel-in-progress": False},
