...
3. ~~governance 룰셋 설계 충돌(et11)~~ — **실측 결과 이미 해소**(2026-09-24 05:2xZ 확인): shared `fcbc807`(2026-08-23 "fix(release): align ET11 gate with sealed GitOps authority")가 `_validate_governance_ruleset` 을 gitops `verify_gitops_write_authority.py` 와 같은 형상(`update` 규칙 단일 + App bypass + 관측 가능 bypass 증명 `current_user_can_bypass`)으로 맞췄고, 라이브 룰셋(21194270 update·21194269 integrity)도 그 형상이며, 오늘 migration-release `35941728847` 의 "Verify the exact App scope and effective main rulesets" 단계가 success. 8/22 핸드오프 §1 항목은 그 뒤 문서에 그대로 복사돼 왔다(Handoffs lag).
4. **S3 웹 재구성 — P1 구현 계획 작성됨**: `plans/2026-09-24-s3-p1-semantic-tokens-2-0-0.md`(토큰 계약 2.0.0: 반경 4/6/8/6/12 · 밀도 30/8/24 · 본문 폭 1120 · 헤더 56 · `rail*→header*` · 테마 30px 컨트롤 · browser-ux 24px · DESIGN.md · design-sync · 홈 미러). 열린 결정 = **홈 미러(Task 9)**: 랜딩이 `--dp-radius-*`·`--dp-color-rail-*`·`--dp-layout-content-max` 를 실제로 쓰므로 미러를 2.0.0 으로 맞추면 랜딩 시각이 바뀌고 홈 시각 기준선 재승인(사람)이 필요하다(A안) / 미러를 1.1.0 에 동결(B안). P2(DpWebShell)~P5 계획은 P1 머지 뒤 각각 따로 쓴다.
