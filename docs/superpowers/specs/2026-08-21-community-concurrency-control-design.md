...
처음에는 "투표 스레드가 500ms 동안 막혀 있음" 을 락의 증거로 삼으려 했다. **그건 락이 있든
없든 참이다** — 락이 없어도 투표 경로는 자기 `UPDATE community_answers` 에서 관리자의 미커밋
UPDATE 에 막힌다.

**판별하는 것은 결과다**: 커밋이 풀린 뒤 투표가 404 를 받고 평판이 0 으로 남는가. 락이 없으면
투표의 UPDATE 가 상태 조건 없이 덮어써 평판이 10 으로 복원된다.

그렇다고 "막혔다" 단언을 버리지 않는다. **역할이 다르다** — 그것은 락의 증거가 아니라
**인터리빙이 실제로 일어났다는 증거**다. 없으면 투표 스레드가 커밋 *이후에* 통째로 실행돼도
404 가 나오고, 테스트는 옳은 이유가 아닌 이유로 통과한다. 단언 두 개가 서로 다른 일을 한다.
...
|---|---|---|---|
| **R1-답변** | `hideAnswer` 열어 둠 → `voteAnswer` 던짐 | 투표 성공, 평판 **10** 으로 복원 | 404, 평판 **0** |
...
1. 경쟁 테스트 **5 건**이 green 이고, `findByIdForUpdate` → `findById` 로 되돌리면 §5 표의
   「락 없으면」 열대로 **red**
