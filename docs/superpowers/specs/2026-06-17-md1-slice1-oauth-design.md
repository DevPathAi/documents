...
| D-9 | API user 객체 shape(프론트 계약) | platform `/auth/refresh`·`/users/me`의 user 객체 = dp_core `User`(`{id, email, nickname, role, onboardingStatus}`)와 동일 키·타입. `id`는 **문자열**. 04_API §1.1 예시(plan 포함)보다 실제 소비자(dp_core `User`)를 따른다. 플랜 작성 중 frontend(빌드4)와 platform 2a `UserSummary` 불일치 발견 → 2a를 이 shape로 정렬(2a 플랜 반영 완료). |
